...
| RandomStep | SandBox用。配置中・開始前は丸い1・2・3を備えた3×1で予約表示し、開始後は抽選幅へ右側から縮む。残る足場数と黒い番号ノード数が一致する |
| UpwardDropdown | SandBox用。1×1のA／B／C表示をタップすると上方向へ3×2に展開し、候補選択後に閉じる |
| Popup | SandBox用。3×1の「ポップアップを開く」を押すと、上方向へ5×2のウィンドウ足場を追加し、×で閉じる |
...
SandBoxでは既存5種類に加えて`RandomStep`、`UpwardDropdown`、`Popup`を各1個配置できます。任意のブロックを1個配置するとブロック一覧に代わってSTARTが表示されます。パネルスイッチで一覧へ戻れば、残りのブロックも追加配置できます。
