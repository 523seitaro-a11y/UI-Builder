...
/// <summary>
/// このスクリプトを付けたスプライトを押している間、プレイヤーが右へ進みます。
/// スプライトには、クリック判定に使う Collider2D が必要です。
/// </summary>
public class MoveR : MonoBehaviour, BlockManager.IBlockOperationState
{
    public bool IsOperating => isPressed;

    [Header("右移動の設定")]
    [Tooltip("右へ動かすプレイヤーの Rigidbody2D を指定します。")]
    [SerializeField] private Rigidbody2D playerBody;

    [Tooltip("長押し中の右方向への移動速度です。値を大きくすると速く進みます。")]
    ...
    public void BeginOperation()
    {
        if (playerBody == null)
        {
            Debug.LogWarning("MoveR: プレイヤーの Rigidbody2D が設定されていません。", this);
            return;
        }
    ...
    private void FixedUpdate()
    {
        ...
        // 右側の壁に触れているときは、壁へ押し付ける速度を与えません。
        // 上下速度には触れないため、壁際でもジャンプや落下が止まりません。
        if (IsTouchingWallInMoveDirection(Vector2.right))
        {
            StopHorizontalMovement();
            return;
        }

        // 落下やジャンプの上下速度は保ったまま、右方向へ一定速度で移動します。
